...
@Component({
  ...
  styleUrl: './current-weather-sidebar.component.css',
})
export class CurrentWeatherComponent {
  ...
  @Input() searchResults: LocationSearchResult[] = [];
  ...
  get resolvedIconPath(): string {
    return this.currentWeather.available && this.currentWeather.iconPath
      ? this.currentWeather.iconPath
      : '/images/weather-states/not-available.svg';
  }
  get temperature(): string {
    return formatTemperature(this.currentWeather.temperatureCelsius, this.unit);
  }
  get conditionVisual() {
    return weatherStateVisuals[this.currentWeather.condition];
  }
